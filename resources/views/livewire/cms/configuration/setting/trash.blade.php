        ...
        <div class="card-body">
            <div class="row">
                ...
                @can('Setting Restore')
                    @if ($settings->count())
                        <div class="col-12 col-sm-auto mb-3">
                            <x-button.restore-all :href="route('cms.configuration.setting.restore-all')" />
                        </div>
                    @endif
                @endcan
                ...
                @can('Setting Delete Permanent')
                    @if ($settings->count())
                        <div class="col-12 col-sm-auto mb-3">
                            <x-button.delete-permanent-all :href="route('cms.configuration.setting.delete-permanent-all')" />
                        </div>
                    @endif
                @endcan
            </div>
                            ...
                            <tr wire:key="{{ $setting->id }}">
                                ...
                                <td>
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        @can('Setting Restore')
                                            <li>
                                                <x-button.restore :class="'dropdown-item'" :href="route('cms.configuration.setting.restore', [
                                                    'setting' => $setting->id,
                                                ])" />
                                            </li>
                                        @endcan
                                        @can('Setting Delete Permanent')
                                            <li>
                                                <x-button.delete-permanent :class="'dropdown-item'" :href="route('cms.configuration.setting.delete-permanent', [
                                                    'setting' => $setting->id,
                                                ])" />
                                            </li>
                                        @endcan
