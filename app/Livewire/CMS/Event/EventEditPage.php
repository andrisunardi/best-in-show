...
class EventEditPage extends Component
{
    ...
    public function resetFields()
    {
        ...
        $this->name_idn = $this->event->name_idn;
    ...
    {
        return [
            'name' => "required|string|max:100|unique:events,name,{$this->event->id}",
            'name_idn' => "required|string|max:100|unique:events,name_idn,{$this->event->id}",
    ...
    {
        $event = (new EventService())->edit(event: $this->event, data: $this->validate());

        $this->flash('success', trans('index.edit_success'), [
            'html' => trans('index.event')." - {$event->id} - ".trans('index.edited'),
        ]);
